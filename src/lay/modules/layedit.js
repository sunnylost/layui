...
layui.define(['layer', 'form'], function(exports) {
    ...
    let MOD_NAME = 'layedit'
    let SHOW = 'layui-show'
    let ABLED = 'layui-disabled'
    let Edit = function() {
        let that = this
        that.index = 0
    ...
    Edit.prototype.sync = function(index) {
        let iframeWin = getWin(index)
        if (!iframeWin[0]) return
        let textarea = $('#' + iframeWin[1].attr('textarea'))
    ...
    Edit.prototype.getSelection = function(index) {
        let iframeWin = getWin(index)
        if (!iframeWin[0]) return
        let range = Range(iframeWin[0].document)
        return document.selection ? range.text : range.toString()
    }

    //iframe初始化
    let setIframe = function(editor, textArea, set) {
            let that = this,
        ...
        //快捷键处理
        hotkey = function(iframeWin, iframe, textArea) {
            let iframeDOM = iframeWin.document,
                body = $(iframeDOM.body)
            ...
            body.on('keydown', function(e) {
                ...
                //处理回车
                if (keycode === 13) {
                    let range = Range(iframeDOM)
                    let container = getContainer(range),
        ...
        filter = function(body) {
            ...
            body.find('*[style]').each(function() {
                let textAlign = this.style.textAlign
        ...
        insertInline = function(tagName, attr, range) {
            ...
            if (iframeDOM.selection) {
                ...
                //非IE
                let text = range.toString() || attr.text
                ...
                toolEvent = {
                    ...
                    image: function(range) {
                        ...
                        layui.use('upload', function(upload) {
                            let uploadImage = set.uploadImage || {}
                            upload.render({
                                url: uploadImage.url,
        ...
        link = function(options, callback) {
            ...
            link.index = layer.open({
                ...
                success: function(layero, index) {
                    let eventFilter = 'submit(layedit-link-yes)'
                    form.render('radio')
        ...
        code = function(callback) {
            ...
            code.index = layer.open({
                ...
                success: function(layero, index) {
                    let eventFilter = 'submit(layedit-code-yes)'
                    form.render('select')
                    layero.find('.layui-btn-primary').on('click', function() {
                        layer.close(index)
                        body.focus()
