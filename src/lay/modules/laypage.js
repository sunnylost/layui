...
layui.define(function(exports) {
    ...
    Page.prototype.view = function() {
        ...
        let index =
            ...
            views = {
                ...
                //每页条数
                limit: (function() {
                    let options = ['<span class="layui-laypage-limits"><select lay-ignore>']
        ...
        return [
            '<div class="layui-box layui-laypage layui-laypage-' +
                (config.theme ? (/^#/.test(config.theme) ? 'molv' : config.theme) : 'default') +
                '" id="layui-laypage-' +
                config.index +
                '">',
            plate.join(''),
        ...
        for (let i = 0, len = childs.length; i < len; i++) {
            if (childs[i].nodeName.toLowerCase() === 'a') {
                laypage.on(childs[i], 'click', function() {
                    let curr = this.getAttribute('data-page') | 0
    ...
    Page.prototype.render = function(load) {
        ...
        let elem = doc[id]('layui-laypage-' + config.index)
        that.jump(elem)
